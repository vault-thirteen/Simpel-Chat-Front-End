package settings

import (
	"errors"

	"github.com/vault-thirteen/Simpel-Chat-Front-End/src/helper"
)

type FrontEndMainSettings struct {
	HostName             string `json:"hostName"`
	PortNumber           uint16 `json:"portNumber"`
	CertFile             string `json:"certFile"`
	KeyFile              string `json:"keyFile"`
	AssetsFolder         string `json:"assetsFolder"`
	HttpContentCacheTime int    `json:"httpContentCacheTime"`
}

func (s *FrontEndMainSettings) Validate() (err error) {
	if s == nil {
		return errors.New(helper.Err_NullPointer)
	}

	if len(s.HostName) == 0 {
		return helper.NewError_ParameterIsNotSet("host name")
	}

	if s.PortNumber == 0 {
		return helper.NewError_ParameterIsNotSet("port number")
	}

	if len(s.CertFile) == 0 {
		return helper.NewError_ParameterIsNotSet("certificate file")
	}

	if len(s.KeyFile) == 0 {
		return helper.NewError_ParameterIsNotSet("key file")
	}

	if len(s.AssetsFolder) == 0 {
		return helper.NewError_ParameterIsNotSet("assets folder")
	}

	if s.HttpContentCacheTime < 0 {
		return helper.NewError_ParameterIsNotSet("HTTP cache time")
	}

	return nil
}
